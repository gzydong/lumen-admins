...
class Tree
{
    public function init($config)
    {
        ...
        if ($id = Arr::get($config, 'pk')) {
            $this->pk = $id;
        }

        if ($array = Arr::get($config, 'array')) {
            $this->array = $array;
        }
        ...
        if ($space = Arr::get($config, 'space')) {
            $this->space = $space;
        }

        if ($parentIdName = Arr::get($config, 'parentIdName')) {
            $this->parentIdName = $parentIdName;
        }

        if ($childrenName = Arr::get($config, 'childrenName')) {
                ...
                if ($maxLevel === 0 || ($maxLevel !== 0 && $maxLevel > $level)) {
                    ...
                    if ($sort) {
                        $this->arraySort(
                            ...
                            $this->pk,
                            Arr::get($sort, 'sort_key', $this->pk),
                            Arr::get($sort, 'sort_type', SORT_ASC)
                        );
                    }
                }
            }
